...
%>
        ...
        <main>
            <div class="mainWrap">
                <section class="sec1">
                    ...
                    <table class="mainboard bd3" style="width:100%;">

                        <thead class="board titel">
                    	<tr>
                            <th>번호</th>
                            <th>제목</th>
                            <th>조회수</th>
                        ...
                        <tbody>
                        <tr class="board text" style="text-align:center; cursor:pointer;" onclick="location.href='noticedetail.jsp?no=<%=notice.getBoards_no() %>'">
                            <th scope="row"><%=notice.getBoards_no() %></th>
                            <td><%=notice.getBoards_title() %></td>
                            <td><%=notice.getView_cnt() %></td>
                            <td><%=notice.getBoards_regdate() %></td>
                        </tr>             
						<%}%>
                        <tr><td colspan=4 class="paging">
 ...
 <%
	...
	if (block_nbr > 1) {
		out.print ("&nbsp[<a href='test.jsp?pageno=1'>" + "맨처음</a>]&nbsp");
	
		//이전 블록 시작 페이지
		previous_block_start_pageno = block_start_page_no - PAGE_PER_BLOCK;
		out.print ("&nbsp[<a href='test.jsp?pageno=" + previous_block_start_pageno + "'>이전</a>]&nbsp");
	}
	...
	for (int pgn = block_start_page_no; pgn <= block_endpage_no; pgn++) {
		...
		if (pgn == cur_page_no) {
			...
			out.print("&nbsp[" + "<a href='test.jsp?" + "pageno=" + pgn + "'>" + pgn + "</a>]&nbsp");
		}
	}
	...
	if (block_endpage_no < nbr_of_page) {
		...
		out.print ("&nbsp[<a href='test.jsp?pageno=" + next_block_start_pageno + "'>다음</a>]&nbsp");
		
		out.print ("&nbsp[<a href='test.jsp?pageno=" + nbr_of_page + "'>맨끝</a>]&nbsp");
	}
